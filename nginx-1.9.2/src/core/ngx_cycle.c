...
listening数组中了。这一步骤就是按照listening数组中的每一个 ngx_listening_t 元素设置socket句柄并监听端口（实际上，这一步骤的主要工作就是调
用表8-2中的 ngx_open_listening_sockets 方法）。
*/
    if (old_cycle->listening.nelts) {
        ls = old_cycle->listening.elts; //旧的listen,如热启动继承过来的sock,见 ngx_add_inherited_sockets
        ...
        for (n = 0; n < cycle->listening.nelts; n++) {
            ...
            // 下面这个for循环不用展开,是废弃代码
            for (i = 0; i < old_cycle->listening.nelts; i++) {
                // 这个for循环废了, 旧的fd的 ls[i].remain 在前面被强制置为0了
                if (ls[i].ignore) {
                    continue;
                }
                if (ls[i].remain) {
